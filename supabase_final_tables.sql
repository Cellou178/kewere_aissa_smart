...
-- ── plans : juste ALTER + UPDATE (la table existe déjà) ───────
...
UPDATE plans SET description='Plan gratuit — 1 ferme, 3 utilisateurs',
    nb_fermes_max=1, nb_users_max=3 WHERE nom='gratuit';
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    type           VARCHAR(30)  NOT NULL DEFAULT 'depense',
...
SELECT table_name, COUNT(*) AS nb_lignes FROM (
    ...
    SELECT 'alertes'          AS table_name FROM alertes          UNION ALL
    SELECT 'plans'            AS table_name FROM plans            UNION ALL
