...
class _AddCycleScreenState extends State<AddCycleScreen> {
  ...
  Future<void> _loadFermes() async {
    ...
    setState(() {
      _fermes = f is List ? f : [];
      if (_fermes.isNotEmpty) {
        final ids = _fermes.map((f) => f['id']?.toString()).toList();
        if (_fermeId == null || !ids.contains(_fermeId)) {
  ...
  Future<void> _submit() async {
    ...
    bool ok;
    if (_isEdit) {
      ok = await ApiService.updateCycle(
          widget.cycle!['id'].toString(), data);
    } else {
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                const Text('Date de fin (optionnelle)',
  ...
  Widget _buildStep3() => Column(children: [
    if (_fermes.isNotEmpty) _card(
      ...
      child: DropdownButtonFormField<String>(
          value: _fermes.any(
                  (f) => f['id']?.toString() == _fermeId)
              ? _fermeId
              : _fermes.first['id']?.toString(),
