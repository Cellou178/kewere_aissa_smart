...
class _CyclesScreenState extends State<CyclesScreen>
  ...
  (c['nom'] ?? '').toLowerCase().contains(_recherche.toLowerCase()) ||
      (c['souche'] ?? '').toLowerCase().contains(_recherche.toLowerCase())).toList();
  ...
  _donnees.where((d) => d['cycle_id']?.toString() == id).toList();

  int _mortaliteCycle(Map c) => _donneesForCycle(c['id']?.toString())
  ...
  Widget build(BuildContext context) {
    ...
    final totalMorts = _cycles.fold<int>(0, (s, c) => s + _mortaliteCycle(c));
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            Row(children: [
              ...
              _headerStat('${tauxGlobal.toStringAsFixed(1)}%', 'Mortalité',
                  Icons.warning_rounded,
                  tauxGlobal > 5 ? Colors.redAccent : Colors.greenAccent),
            ]),
        ...
        Expanded(child: _loading
          ...
          _buildList(_cyclesFiltres, 'Aucun cycle', 'Créez votre premier cycle'),
        ])),
      ]),
    );
  }
  ...
  Widget _cycleCard(Map c) {
    ...
    final color = isActif ? kGreen : isTermine ? Colors.blueGrey : kOrange;
    final sujets = ((c['nombre_sujets'] ?? 0) as num).toInt();
    final morts = _mortaliteCycle(c);
    ...
    final tauxColor = taux > 5 ? kRed : taux > 2 ? kOrange : kGreen;
      ...
      child: Container(
        ...
        child: Column(children: [
            ...
            child: Row(children: [
              Container(width: 44, height: 44,
                  ...
                  child: Icon(isActif ? Icons.pets_rounded
                      : isTermine ? Icons.check_circle_rounded
          ...
          Padding(
            ...
            child: Column(children: [
              // Barre de progression mortalité
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text('Mortalité: $morts/$sujets',
                    style: const TextStyle(fontSize: 11, color: Colors.grey)),
                Text('${taux.toStringAsFixed(1)}%', style: TextStyle(
                    color: tauxColor, fontWeight: FontWeight.w700, fontSize: 11)),
              ]),
              const SizedBox(height: 4),
              ClipRRect(borderRadius: BorderRadius.circular(3),
                  child: LinearProgressIndicator(
                      value: sujets > 0 ? (morts / sujets).clamp(0.0, 1.0) : 0,
              ...
              // Chips infos
              Row(children: [
                _chip(Icons.pets_rounded, '$sujets sujets', kBlue),
          ...
          Expanded(child: SingleChildScrollView(
            ...
            child: Column(children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 8),
                _detailKpi('📊', '${taux.toStringAsFixed(1)}%', 'Taux mort.',
                    taux > 5 ? kRed : kGreen),
                const SizedBox(width: 8),
                _detailKpi('🌡️', '${temp.toStringAsFixed(1)}°', 'Temp. moy.',
                    kOrange),
              ]),
              const SizedBox(height: 12),
  ...
  Widget _detailKpi(String emoji, String value, String label, Color color) =>
